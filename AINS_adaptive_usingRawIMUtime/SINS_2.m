...
function [Simulation]=SINS_2(Simulation , I,I2 , dt ,func_f, fc_f_RP , fc_f_accel , fc_f_gyro , filtered_accel , filtered_gyro ,ave_sample,muu,coeff_final,include_MA)
...
%             end
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [gl,g0] = Gravity(Simulation.Output.INS.X_INS(I2-ave_sample,1:3));

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [ Simulation ] = Digital_Filter( Simulation , fb    , dt , fc_f_RP    , I2 , 'RollPitch', ave_sample );
            [ Simulation ] = Digital_Filter( Simulation , fb    , dt , fc_f_accel , I2 , 'Accel', ave_sample);
            [ Simulation ] = Digital_Filter( Simulation , Wib_b , dt , fc_f_gyro  , I2 , 'Gyro', ave_sample);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            filtered_RP    = Simulation.Output.INS.FilteredSignal.filtered_RP(I2- ave_sample + 1,1:3);
            filtered_Accel = Simulation.Output.INS.FilteredSignal.filtered_Accel(I2- ave_sample + 1,1:3);
            filtered_Gyro  = Simulation.Output.INS.FilteredSignal.filtered_Gyro(I2- ave_sample + 1,1:3);
            filtered_Accel_ = Simulation.Output.INS.FilteredSignal.filtered_Accel(I2 - ave_sample ,1:3);
            filtered_Gyro_  = Simulation.Output.INS.FilteredSignal.filtered_Gyro(I2- ave_sample ,1:3);  
            Simulation.Output.INS.norm.Accel_norm(I2- ave_sample + 1) = abs(norm(filtered_RP) - norm(gl));%fb
            Simulation.Output.INS.norm.Gyro_norm (I2- ave_sample + 1) = norm(filtered_Gyro);%Wib_b               
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
             X_ = Simulation.Output.INS.X_INS(I2 - ave_sample,1:9);
             C_ = Simulation.Output.INS.Cbn(:,:,I2 - ave_sample);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if     ~filtered_accel && ~filtered_gyro
                    [fb,fb_,Wib_b,Wib_b_]=correction_Bias(Simulation,fb,fb_,Wib_b,Wib_b_,I2,ave_sample);
                elseif filtered_accel && ~filtered_gyro
                    [fb,fb_,Wib_b,Wib_b_]=correction_Bias(Simulation,filtered_Accel,filtered_Accel_,Wib_b,Wib_b_,I2,ave_sample);
                elseif ~filtered_accel && filtered_gyro
                    [fb,fb_,Wib_b,Wib_b_]=correction_Bias(Simulation,fb,fb_,filtered_Gyro,filtered_Gyro_,I2,ave_sample);
                elseif filtered_accel && filtered_gyro
                    [fb,fb_,Wib_b,Wib_b_]=correction_Bias(Simulation,filtered_Accel,filtered_Accel_,filtered_Gyro,filtered_Gyro_,I2,ave_sample);   
                end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         end 
            ...
            Simulation.Output.INS.Cbn(:,:,I2 - ave_sample + 1) = InCBN(INS_Euler_I);%corrected transformation matrix             
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            Simulation.Output.INS.X_INS(I2 - ave_sample + 1,:) = [ X , Simulation.Output.INS.X_INS(I2 - ave_sample,10:15)];
            Simulation.Output.INS.fnn(I2 - ave_sample ,:)   = fnn;
            Simulation.Output.INS.fn(I2 - ave_sample + 1,:)    = X_dot(4:6,:)';
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %         [ Simulation ] = Alignment3( Simulation , filtered_RP , gl(3) , I , muu , ave_sample );%fb               
                [ Simulation ] = Alignment4( Simulation , filtered_RP , gl(3) , I2 , muu , ave_sample );%fb   
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
