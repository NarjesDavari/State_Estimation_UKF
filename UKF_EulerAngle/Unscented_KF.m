function [ Simulation ] = Unscented_KF( Simulation , I  , fc_f_RP , fc_f_accel , fc_f_gyro , dt , ...
                                       filtered_accel , filtered_gyro , func_f , Selection_Param , C_DVL_IMU ,  WM ,WC , W ,sigma,W2,WM2,WC2,alpha,beta, c ,SF, ave_sample)
    ...
    C_ = Simulation.Output.INS.Cbn(:,:,I-1);
    Qcc = Simulation.Output.Kalman_mtx.Qc;
    Qc = diag([Qcc.q_ax,Qcc.q_ay,Qcc.q_az,Qcc.q_wx,Qcc.q_wy,Qcc.q_wz,Qcc.q_Bax,Qcc.q_Bay,Qcc.q_Baz,Qcc.q_Bwx,Qcc.q_Bwy,Qcc.q_Bwz]);
    if     ~filtered_accel && ~filtered_gyro
        [X , P , S, Y_i] = UKF_predict(X_ , P_ , func_f , G_ , Q_ ,Qc, C_ , fb , fb_ , Wib_b , Wib_b_ , gl , dt , WM ,WC , W ,sigma,W2,WM2,WC2,alpha,beta, c,I);
    elseif filtered_accel && ~filtered_gyro
        [X , P, S, Y_i] = UKF_predict(X_ , P_ , func_f , G_ , Q_ ,Qc, C_ , filtered_Accel , filtered_Accel_ , Wib_b , Wib_b_ , gl , dt , WM ,WC , W ,sigma,W2,WM2,WC2,alpha,beta, c,I);
    elseif ~filtered_accel && filtered_gyro
        [X , P, S, Y_i] = UKF_predict(X_ , P_ , func_f , G_ , Q_  ,Qc, C_ , fb , fb_ , filtered_Gyro , filtered_Gyro_ , gl , dt , WM ,WC , W ,sigma,W2,WM2,WC2,alpha,beta, c,I);
    elseif filtered_accel && filtered_gyro
        [X , P, S, Y_i] = UKF_predict(X_ , P_ , func_f , G_ , Q_  ,Qc, C_ , filtered_Accel , filtered_Accel_ , filtered_Gyro , filtered_Gyro_ , gl , dt , WM ,WC , W ,sigma,W2,WM2,WC2,alpha,beta, c,I);
    end
    INS_Euler_I                      = [X(7),X(8),X(9)] ;
    Simulation.Output.INS.Cbn(:,:,I) = InCBN(INS_Euler_I);%Predicted transformation matrix
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [ Simulation ] = UKF_Correction_Param(Simulation,Selection_Param,I,C_DVL_IMU, SF,ave_sample );    
...
%         [X , P]                   = UKF_update1(X , P , Y , H , R, I );
         [Simulation,X , P]                   = UKF_update2(Simulation,X , P,S, Q_ , Y , H , R, Y_i , WM , WC ,W , c, W2,WM2,WC2,sigma,alpha,beta,I );
%         [X , P]                   = UKF_update3(X , P , Y ,Y_i, H , R  ,sigma,W2,WM2,WC2,alpha, I);
        INS_Euler_I                      = [X(7),X(8),X(9)] ;
        Simulation.Output.INS.Cbn(:,:,I) = InCBN(INS_Euler_I);%Correted transformation matrix        
    end
